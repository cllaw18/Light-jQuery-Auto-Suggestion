...
var sugMenuW               = "600px";                //Width of Search input box and Suggest menu, 
var jsonKey1               = "key_text";             //json Frame Key 1
var jsonKey2               = "desc_text";            //json Frame Key 2
...
function searchAndSuggest(obj, keyword) {
    ...
    for (var i=0; i<objSize;i++) {
        if (obj[i][jsonKey1].search(keyword) != -1) {
            resultArr.push(obj[itemCounter][jsonKey1]);
            var tempParams = '\''+obj[i][jsonKey1]+'\'';
            tempStr += '<li onclick="selectsug('+tempParams+')">'+obj[i][jsonKey1]+'    <span style="color:#aaa">'+obj[i][jsonKey2]+"</span></li>";
            itemCounter++;
